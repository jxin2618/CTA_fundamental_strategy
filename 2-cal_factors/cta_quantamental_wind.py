# -*- coding: utf-8 -*-
"""
Created on Tue Dec 22 14:11:13 2020
...
class Cal_Factors(object):
    ...
    def __init__(self, option, condition):
        ...
        self.begin_date = pd.Timestamp('2013-01-01')
    ...
    # 原始数据读取
    def read_data(self, index_code, freq):
        os.chdir('D:/Xin/Program/metal/wind_data/')
    ...
    def quantile_zscore_ttm(self, factor_dictionary, raw_data, calendar, later_1y):
        ...
        df_ttm = pd.DataFrame(data = raw_data.loc[later_1y, :].values / df_ttm.values, columns=['data_value_ttm'], index=df_ttm.index)
...
for condition in ['with_lag', 'without_lag']:
    ...
    ## raw_data_list
    file_data = 'D:/Xin/Program/metal/钢联有色数据1.xlsx'
    data_name_list = pd.read_excel(file_data, sheet_name='wind')
    ...
    for index in index_list:
        ...
        data = cal_factors.read_data(index, freq)
